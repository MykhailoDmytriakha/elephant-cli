...
def files_block(W):
    ...
    for path, what in (("init/request.md", "запрос пользователя — его словами, только о задаче"),
                       ("records.jsonl", "все ступени всех фаз и все потоки — по записи на строку, ничего не переписывается"),
                       ("checks.jsonl", "реестр обещаний и вердиктов — что должно получиться и чем проверено"),
                       ("journal.jsonl", "события: фаза перешла · узел начат · слово сказано"),
                       ("records.jsonl#stages · checks.jsonl@stage", "карта этапов записями, обещания этапов в реестре; сеть считается"),
...
def big_picture(mode, W):
    ...
    for line in (
        ...
        "автономия: человек сказал «работай сам» — el grant \"<его слова>\"; в его место решают (--assumed), "
