...
def scenario(ws):
    ...
    add(S(["context", "--full"], label="context --full: the whole picture on demand"))
    add(S(["context"], env={"ELEPHANT_SCREEN": "2000"}, label="context over the screen budget: a table of contents"))
    add(S(["context", "--full"], env={"ELEPHANT_SCREEN": "2000"}, label="context --full over budget: the whole, with the warning"))
    ...
    add(S(["context"], env={"ELEPHANT_SCREEN": "2000"}, label="context over budget, sections on disk: the index rows"))
    ...
    # THE STAGE LAW (owner, 2026-08-26): a stage is laid out before it starts, the layout gets
    # his word over the stage, the packages start — the stage itself never does (soft: refused)
    add(S(["plan", "new", "s3", "проверка на живом"]))
    add(S(["plan", "set", "s3", "result", "сценарий пройден на телефоне"]))
    add(S(["plan", "set", "s3", "check", "- список пришёл в чат"]))
    add(S(["plan", "start", "s3"], rc=1, label="plan start s3: a stage is not laid out — refused"))
    ...
    add(S(["grant", "change", "x", "--hours", "2"], rc=1, label="grant change: nothing to change"))
    add(S(["grant", "end", "x"], rc=1, label="grant end: nothing to end"))
    add(S(["grant", "работай сам, без меня", "--no", "push, деньги", "--until", "приёмка",
           "--name", "первый срез", "--hours", "2"]))
    add(S(["grant", "x", "--hours", "abc"], rc=1, label="grant --hours: not a number"))
    add(S(["grant"], label="grant: the state"))
    add(S(["grant", "change", "давай четыре часа, не два", "--hours", "4"], label="grant change: the owner's correction"))
    add(S(["grant", "change", "слова без изменения"], rc=1, label="grant change: nothing changed"))
    ...
    add(S(["review"], label="review: the grant with two decisions"))
    ...
    add(S(["review"], label="review: the grant on hold, decisions as history"))
    add(S(["grant", "продолжай"], label="grant: «продолжай» — a new grant after the hold"))
    add(S(["status"]))
    add(S(["grant", "end", "дошёл: срез 8.0 → 6.1 ГБ"], label="grant end: the natural end"))
    add(S(["halt", "x"], label="halt after end: nothing to stop"))
    add(S(["grant", "ещё раз, до вечера", "--until", "вечер"], label="grant: another period"))
    add(S(["halt", "стоп, дальше я сам", "--by", "user"], label="halt --by user: taken back by the owner"))
    add(S(["grant", "и снова сам"], label="grant: over an ended one"))
    add(S(["grant", "поверх — новые условия", "--hours", "1"], label="grant over a standing one: replaced"))
    add(S(["review"], label="review: five grants, every kind of end"))
