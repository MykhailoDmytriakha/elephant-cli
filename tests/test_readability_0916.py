...
buried under the breakdown; (2) `todo done` accepted «Commit a1b2c3d4e5 removed regionCode == EU check
...
class OutcomeIsForTheOwner(Base):
    def test_a_code_trace_in_the_words_is_named_not_refused(self):
        code, out, err = run("todo", "done", "1.1", "file:docs/x.md", "Commit a1b2c3d4e5 (API merge) removed regionCode == EU check and gutted update()")
        self.assertEqual(code, 0, err)
        self.assertIn("the words of done read like a code trace", err)
        self.assertIn("ref:a1b2c3d4e5", err, "the hash is a trace — it is proof, not the words")
    ...
    def test_plain_words_pass_in_silence(self):
        code, out, err = run("todo", "done", "1.1", "ref:a1b2c3d4e5", "the router was disconnected on purpose in the API merge: the EU check is gone")
        self.assertEqual(code, 0, err)
