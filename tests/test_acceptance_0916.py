"""Report of 2026-09-16 18:47 from a live coding case (el 1.16.0): phase 4 closed after a CSET baseline and a
manual computation; 5.5 closed on HTTP 200 with benefits; the DEV trace then showed no PremierPricing call and
...
GOAL = ("discount applied end to end [run: trace shows outbound PremierPricingDiscLookup] "
...
class AcceptanceIsItems(Base):
    def test_open_hints_the_criteria_and_order_names_the_uncovered_one(self):
        ...
        self.assertIn('--expect "[run: trace shows outbound PremierPricingDiscLookup]"', out)
        run("todo", "add", "1", "see the outbound call in the trace", "--expect", "[run: trace shows outbound PremierPricingDiscLookup]")
        order = run()[1].split("## Order")[1]
        self.assertNotIn("[run: trace shows outbound PremierPricingDiscLookup] and no item", order)
    ...
    def test_close_refuses_over_an_unproved_promise_and_a_baseline_does_not_count(self):
        ...
        run("todo", "add", "1", "see the outbound call", "--expect", "[run: trace shows outbound PremierPricingDiscLookup]")
        run("todo", "add", "1", "discount fields", "--expect", "[run: discount fields non-zero]")
        run("todo", "add", "1", "build the CSET baseline")  # a step, no criterion
        run("todo", "done", "1.3", "run:curl router -> HTTP 200, five cost rows", "baseline found")
        run("todo", "done", "1.1", "run:grep trace -> outbound PremierPricingDiscLookup groupNbr=000009641", "the call is there")
        self.gates_ok()
        ...
        self.assertIn("- phase promise: 3 of 3 proved — [run: trace shows outbound PremierPricingDiscLookup] by 1.1 · "
