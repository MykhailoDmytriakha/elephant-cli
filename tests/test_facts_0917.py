...
builds on («the router never calls Partner Pricing in the CART flow»). The fact chain is a search tree: branches
...
class FactPocket(Base):
    def test_expected_fact_then_verdict_at_done(self):
        code, out, err = run("todo", "add", "1", "grep the DEV trace", "--expect", "[run: trace shows the outbound call]",
                             "--fact", "the router calls Partner Pricing for the pair")
        ...
        self.assertIn("expected the fact «the router calls Partner Pricing for the pair» — what is established now?", err)
        self.assertIn("--fact confirmed", err)
        code, out, err = run("todo", "done", "1.1", "run:grep trace -> no outbound call", "trace read on three pods",
                             "--fact", "the router never calls Partner Pricing in the CART flow")
        self.assertEqual(code, 0, err)
        self.assertIn("fact 1.1: «the router never calls Partner Pricing in the CART flow» — established; it enters the chain: el facts", out)
        todo = self.read("TODO.md")
        self.assertIn("    - result: trace read on three pods\n      - run: grep trace → no outbound call\n    - fact: the router never calls Partner Pricing in the CART flow\n", todo)
        self.assertIn("· fact: the router never calls Partner Pricing in the CART flow", self.read("JOURNAL.md"))
...
class TheChain(Base):
    def setUp(self):
        ...
        run("todo", "add", "1", "grep the trace", "--expect", "[run: trace]", "--fact", "the router calls Partner")
    ...
    def test_el_facts_renders_the_tree_and_the_entry_counts(self):
        run("todo", "done", "1.1", "run:grep -> no call", "read", "--fact", "the router never calls Partner in the CART flow")
    ...
    def test_refutation_runs_down_the_chain_as_a_question_not_a_reopen(self):
        ...
        self.assertIn("  · 1.1 the router calls Partner", out, "the reopened item keeps its fact as an expectation")
