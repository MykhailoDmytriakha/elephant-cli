...
class ClosedLineIsEls(Base):
    LONG = ("All phases complete: sreForceAllLog successfully disabled across DEV, SIT, UAT, LNP, STG, and PROD; "
...
class ItemsAreRephrasedNotRecounted(Base):
    ...
    ITEM = "Verify logs: trigger endpoint (/v3/mle/test/frt-generator) and confirm sreForceAllLog=false in pod logs"
    ...
    def test_visible_len_exempts_links_only(self):
        ...
        marked = "Verify logs: trigger endpoint `/v3/mle/test/frt-generator` and confirm `sreForceAllLog=false` in pod logs"
    ...
    def test_refusal_suggests_rephrasing_and_the_rephrased_item_fits(self):
        ...
        code, out, err = run("todo", "add", "1", "Trigger /v3/mle/test/frt-generator, confirm sreForceAllLog=false in pod logs")
        self.assertEqual(code, 0, err)
