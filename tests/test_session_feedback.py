"""Regressions from the live-session feedback (2026-09-01, 2026-09-03): editable TODO,
...
class RootCaseCheck(unittest.TestCase):
    ...
    def test_root_case_violations_are_caught(self):
        run("case", "new", "--root", "field trips", "--goal", "g")
        t = Path("TODO.md")
        t.write_text(t.read_text().replace("# TODO — field trips",
                     "# TODO — field trips\n\n- [ ] 2 Calls\n  - [ ] 2.1 " + "и" * 174))
        code, out, err = run("check")
...
class HoldResume(Base):
    ...
    def test_done_clears_hold(self):
        ...
        run("todo", "hold", "1.1", "")
        code, out, err = run("todo", "done", "1.1", "owner", "ok")
        self.assertEqual(code, 0, err)
