"""Feedback of 2026-09-14 14:17 (a live case in root mode): a planned phase that ran alongside
...
class PhaseOutOfTurn(Base):
    ...
    def plan_side_branch(self):
        ...
        code, out, err = run("todo", "done", "4.1", "owner", "closed 2024-01-15, no debt")
    ...
    def test_a_finished_side_branch_closes_from_the_plan(self):
        self.plan_side_branch()
        code, out, err = run("phase", "close", "4", "no debt: the court closed it 2024-01-15")
        ...
        self.assertIn("- [x] 4 Old case — no debt: the court closed it 2024-01-15 ·", todo)
        ...
        self.assertIn("result: no debt: the court closed it 2024-01-15", pf)
...
class OrderSeesEndedItems(Base):
    ...
    def test_every_item_ended_is_named_with_the_close_and_what_it_needs(self):
        ...
        run("todo", "done", "1.1", "owner", "answered")
    ...
    def test_a_planned_phase_out_of_turn_gets_the_same_line(self):
        ...
        run("todo", "done", "2.1", "owner", "no debt")
