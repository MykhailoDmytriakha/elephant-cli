"""The owner's word of 2026-09-14 (a live case, TODO.md on screen): a tick is not a proof. Prove2Me
...
class TheFourKinds(Base):
    ...
    def test_ref_run_and_owner(self):
        code, out, err = run("todo", "done", "1.2", "ref:R000123-000001", "request filed via the portal")
        self.assertEqual(code, 0, err)
...
class SameDoor(Base):
    def test_a_second_done_adds_evidence_to_a_done_item(self):
        ...
        # when given; since 1.21.0 (feedback 2026-09-22) it writes no RESULT: the tick is not new, only its proof
...
class TheTailTravels(Base):
    def test_phase_close_carries_the_tail_into_the_phase_file_links_rebased(self):
        run("todo", "done", "1.1", "file:evidence/receipt.pdf", "paid")
        run("todo", "done", "1.2", "ref:R000123", "filed")
