...
class PhasePromise(Base):
    def test_goal_placeholders_are_validated_and_held_to_at_close(self):
        ...
        run("todo", "add", "1", "write the answer")
        ...
        self.assertIn("- phase promise: 1 of 2 filled — missing [run: curl → 200]\n", pf)
...
class TodoShow(Base):
    def test_the_card_prints_pockets_inputs_and_feeds(self):
        ...
        run("todo", "done", "1.1", "file:research/why.md", "disconnected on purpose in the CSH merge")
